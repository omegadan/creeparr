...
from patreonarr import __version__
from patreonarr.api.router import api_router
from patreonarr.api.system import health_payload
from patreonarr.config import EnvConfig, get_env_config
from patreonarr.core.errors import AppError
from patreonarr.db.migrate import run_migrations
from patreonarr.logging_setup import setup_logging
from patreonarr.patreon.errors import PatreonError
from patreonarr.services import Services, build_services
...
def create_app(env: EnvConfig | None = None, *, start_background: bool = True) -> FastAPI:
    ...
    async def lifespan(app: FastAPI):
        ...
        log.info("Patreonarr %s ready on port %s", __version__, env.port)
        try:
            yield
        finally:
            if start_background:
    ...
    app = FastAPI(title="Patreonarr", version=__version__, lifespan=lifespan, docs_url="/api/docs")
